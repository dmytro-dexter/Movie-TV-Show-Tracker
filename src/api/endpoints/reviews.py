...
def get_review_by_id(review_id: int, db: Session = Depends(get_db)) -> schemas.ReviewBase:
    return crud_reviews.get_review_by_id(review_id=review_id, db=db)


@router.post("", response_model=schemas.ReviewBase)
def create_review(review: schemas.ReviewCreate, movie_id: int, db: Session = Depends(get_db)) -> schemas.ReviewBase:
